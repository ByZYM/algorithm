    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i != 25; i++) {
            for (int value : list) {
                ...
                if (value >= 0) if ((value & 1) == 1) real[0] = 1;
                else real[1] = 1;
                else if ((value & 1) == 1) real[2] = 1;
                else real[3] = 1;
        ...
        System.out.println("训练完毕，下面请输入一个任意数字，神经网络将自动判断它是奇数还是偶数。");
        while (true) {
            ...
            switch (idx) {
                case 0:
                    System.out.format("%d是一个奇数\n", rawVal);//正
                    break;
                case 1:
                    System.out.format("%d是一个偶数\n", rawVal);
                    break;
                case 2:
                    System.out.format("%d是一个奇数\n", rawVal);//负
                    break;
